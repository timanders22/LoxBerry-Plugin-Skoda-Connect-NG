...
> **Fassung 0.9.17 — ungeprüft.** Das Plugin wurde ohne Skoda-Konto und ohne
...
Diese Fassung benutzt deshalb `WachsameRotation` in `bin/skoda.py` — einen
...
nichts, was beides kann; deshalb die eigene Klasse. Der Einmallauf (`--wachzeichen`, Selbsttest) behält seinen einfachen `FileHandler`: ihn startet der Cron bei jedem Lauf neu, er hält keine Datei über Stunden offen.
...
`skoda_start.log`, das bei jedem Start geleert wird; das Protokoll gehört
